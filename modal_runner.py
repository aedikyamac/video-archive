...
def _get_drive_credentials():
    ...
    if raw:
        ...
        if raw.startswith('{'):
            ...
            try:
                path = Path(raw)
                if path.is_file(): return service_account.Credentials.from_service_account_file(str(path), scopes=scopes)
            except Exception as error: print('Error loading service account credentials:', error)
    from google.auth import default as google_auth_default
    creds, _ = google_auth_default(scopes=scopes)
    return creds


def _service_account_credentials(): return _get_drive_credentials()
...
def drive_upload(path: Path, metadata: dict[str, Any]) -> str | None:
    ...
    result = service.files().create(body={"name": path.name, "parents": [folder_id], "description": json.dumps(metadata)}, media_body=MediaFileUpload(str(path), resumable=True), supportsAllDrives=True, fields="id,webViewLink").execute()
...
def github_metadata(record: dict[str, Any]) -> None:
    ...
    with urllib.request.urlopen(request) as response: current = json.load(response)
    ...
    with urllib.request.urlopen(update): pass


def _cobalt_metadata(result: dict[str, Any], source: str) -> dict[str, Any]:
    return {"sourceUrl": source, "title": result.get("title") or result.get("filename") or "Archived video", "uploader": result.get("author") or result.get("uploader") or result.get("artist"), "thumbnail": result.get("thumbnail"), "duration": result.get("duration")}
...
def _download_cobalt(source: str, directory: Path) -> tuple[Path, dict[str, Any]]:
    ...
    except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, json.JSONDecodeError) as error: raise RuntimeError(f"Cobalt request failed: {error}") from error
    ...
    except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, OSError) as error: raise RuntimeError(f"Cobalt media download failed: {error}") from error
    if output.stat().st_size == 0: raise RuntimeError("Cobalt returned an empty media file")
...
def _download_ytdlp(source: str, directory: Path) -> tuple[Path, dict[str, Any]]:
    ...
    for clients in YOUTUBE_PLAYER_CLIENT_CHAINS:
        options = {"outtmpl": str(output), "format": "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", "merge_output_format": "mp4", "noplaylist": True, "quiet": True, "extractor_args": {"youtube": {"player_client": clients}}, "js_runtimes": {"node": {}}}
        try:
            with YoutubeDL(options) as ydl:
                info = ydl.extract_info(source, download=True)
                filename = Path(ydl.prepare_filename(info))
                if not filename.exists(): filename = filename.with_suffix(".mp4")
                return filename, {"sourceUrl": source, "title": info.get("title") or info.get("id"), "uploader": info.get("uploader"), "thumbnail": info.get("thumbnail"), "duration": info.get("duration"), "playerClient": clients}
        except DownloadError as error:
            last_error = error
            print(f"yt-dlp player_client={','.join(clients)} failed; trying next client: {error}")
    if os.getenv("COBALT_API_URL") or os.getenv("USE_COBALT_FALLBACK") == "1":
        return _download_cobalt(source, directory)
    raise RuntimeError(f"All yt-dlp player client chains failed: {last_error}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            if _is_youtube_url(source):
                try: filename, record = _download_ytdlp(source, directory); record["downloader"] = "yt-dlp"
                except Exception as ytdlp_error:
                    if os.getenv("COBALT_API_URL") or os.getenv("USE_COBALT_FALLBACK") == "1":
                        print(f"yt-dlp failed; using configured Cobalt fallback: {ytdlp_error}")
                        filename, record = _download_cobalt(source, directory); record["downloader"] = "cobalt"
                    else: raise
            else:
                try: filename, record = _download_cobalt(source, directory); record["downloader"] = "cobalt"
                except Exception as cobalt_error:
                    print(f"Cobalt downloader failed for {source}: {cobalt_error}; falling back to yt-dlp")
                    filename, record = _download_ytdlp(source, directory); record["downloader"] = "yt-dlp"
